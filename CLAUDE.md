...
Phiên bản: `ALGORITHM = 'VN30_EMA_VWAP_RSI_ATR_5M_V5'` (nhật ký thay đổi nằm ở đầu `derivatives.service.ts`). Nguồn giá: **VN30F1M** (Entrade `/ohlcs/derivative`), nến 5 phút **đã đóng** (tham số `true` truyền vào `fetchIntradayDerivativeOhlc` là `noCache`, không phải `includeForming`), 120 nến gần nhất trong 10 ngày. Giá chia 1000 (đơn vị điểm). Entry = close của nến đã đóng gần nhất (có thể cũ 1–5 phút so với giá lúc gửi tin); `settleOpenDecisions` chỉ xét các nến có giờ bắt đầu sau `decidedAt`, nên bỏ qua ~1 nến đầu tiên sau khi vào lệnh.
...
- **Điều kiện** (5 mỗi phía): LONG = close>VWAP ngày; EMA9>EMA21; slope EMA9 (5 nến)>0; RSI14 ∈[50,75]; close>đỉnh 12 nến trước **và** volume nến cuối ≥1.2× TB20 nến trước.
  SHORT = đối xứng, RSI ∈[25,50]. **Cả LONG và SHORT cần 5/5** (V5; trước đó SHORT 4/5), và điểm phía đó phải lớn hơn phía kia.
- **SL/TP**: SL = entry ∓ 1.2×ATR, TP = entry ± 3×ATR (V5; R:R hiển thị 2.5). Chỉ báo EMA/RSI/ATR tự cài (seed EMA bằng SMA chạy dần, RSI/ATR Wilder), VWAP tính từ đầu ngày, `confidence` khiêm tốn (`min(72, 55+(score−ngưỡng)×8)` ⇒ V5 luôn 55% với LONG/SHORT vì ngưỡng = 5/5; NO_TRADE ≤50%).
- **Thoát** (duyệt các nến sau `decidedAt`): chạm SL/trail trước → chạm TP → cập nhật SL (dời hoà vốn khi lãi ≥1R=1.2×ATR; "trailing" cách đỉnh 1R khi lãi ≥2R, còn hiệu lực vì TP nay 2.5R) → sau 24 nến thoát theo close (`TIME_EXIT`) → 14:45 đóng hết (`closeAllOpenAtEndOfSession`). Trong cùng nến, SL được kiểm tra trước TP (bảo thủ). Outcome theo P/L: >+0.01 WIN, <−0.01 LOSS, còn lại TIME_EXIT.
- **Backtest** (`vn30-backtest-core.ts`, chạy qua queue `derivatives-tasks`, cache Redis 15 ngày theo tháng): `decide()` + `settle()` thuần. Chạy nhiều biến thể (LONG 4/5, LONG siết 5/5 RSI 58–72, SHORT only, LONG only, sáng only <11:30, ATR≥3) trên **chỉ số VN30** (không phải F1M). Từ 2026-09-20 hằng số thoát (TP 3 ATR, 24 nến) và SHORT 5/5 đã khớp V5; cache Redis đổi khoá thành `backtest:vn30:v3`. Có `MAX_DAILY_LOSSES = 2`. Thống kê: WR, PF, expectancy, max drawdown, Sharpe/lệnh, chuỗi thắng/thua, net theo tháng.
...
**Phân tích phái sinh (2026-09-20, đọc DB qua `ANALYTICS_DB_*` = 10.10.0.3, chỉ SELECT):** live V4 trên F1M 141 lệnh NET +26.45đ, PF 1.09, SHORT −16.18đ; trừ chi phí ~0.4đ/lệnh thì gần hoà vốn.
Nến 5m F1M lấy công khai từ Entrade (`/ohlcs/derivative?symbol=VN30F1M&resolution=5`, có từ 2026-03-02) rồi mô phỏng lại đúng logic live (tái hiện ~95% số lệnh live). Từ đó ra V5 (mục 7, changelog trong `derivatives.service.ts`).
Cách kiểm chứng cải tiến phái sinh: chia 2 giai đoạn (trước/sau 2026-07-24), **chỉ tin thay đổi tốt lên ở cả hai**, tính chi phí, kiểm tra vùng lân cận tham số, và so live sau khi chạy (`yarn analytics:derivatives --algo=V5`). Mã mô phỏng chưa được lưu vào repo.
...
- **B3. (ĐÃ SỬA trong V5) Phái sinh: nhánh trailing là mã chết.** Với TP 2 ATR = 1.67R nhưng trailing kích hoạt ở 2R, lệnh luôn thoát ở TP trước; chỉ dời hoà vốn ở 1R có tác dụng. V5 nâng TP lên 3 ATR (2.5R) nên trailing hoạt động (dữ liệu: 40 TP, 51 BE trên 199 lệnh mô phỏng).
- **B4. Backtest phái sinh còn lệch live ở phần vào lệnh**: `decide()` trong `vn30-backtest-core.ts` vẫn không có lọc volume ≥1.2×, `MIN_ATR_POINTS` mặc định 1.5 (service truyền 1.5; live 2.5), không có cổng cửa sổ roll, LONG "bình thường" chỉ cần 4/5 (live 5/5), có `MAX_DAILY_LOSSES=2` (live **không có**), và chạy trên chỉ số VN30 (live dùng F1M). Phần thoát (TP 3 ATR, 24 nến) và SHORT 5/5 đã khớp V5.
- **B5. (ĐÃ ĐÍNH CHÍNH — trước đây ghi sai)** Live phái sinh dùng nến 5 phút **đã đóng**, không phải nến đang hình thành (xem mục 7). Rủi ro thật là **giá vào cũ**: entry = close nến đã đóng gần nhất, lệch 1–5 phút so với lúc gửi tin.
...
- Sửa `public/index.html` / `public/js/*.js` / `public/css/*` thì **tăng `VERSION` trong `public/sw.js`** (app shell được cache-first, không tăng thì người dùng PWA vẫn thấy bản cũ). UI phái sinh hiển thị phiên bản thuật toán lấy từ cột `algorithm` của quyết định mới nhất (`derivAlgoVersion()`).
